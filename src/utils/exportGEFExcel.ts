...
function getRoomsFromRoomsData(
  roomsData: RoomsDataPerPeriod,
  periodId: number,
  dateIso: string,
  slotIndex: number,
  ...
  const periodRooms = roomsData[periodId] || {};
  const cellRooms = periodRooms[key] || {};
  return cellRooms[subjectId]?.rooms || [];
}
...
const roomsFromState = getRoomsFromRoomsData(
  roomsData,
  period.id,
  dateIso,
  slotIndex,
...
const roomsFromCsv =
  roomsMap.get(`${subject.codi}|${dateIso}|${slot.start}|${slot.end}`) || [];

const rooms = roomsFromState.length ? roomsFromState : roomsFromCsv;
